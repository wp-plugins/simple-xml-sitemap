...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
	...
	<url>
	    ...
	    switch($array['post_type']){
	      case 'post';$output .= '<priority>'.$priority['Posts'].'</priority>';break;
	      case 'page';$output .= '<priority>'.$priority['Pages'].'</priority>';break;
	...
	foreach($tags as $tag){
	   $output .= '<url>
		...
		<lastmod>'.date('c').'</lastmod>
		<changefreq>daily</changefreq>
		<priority>'.$priority['Tags'].'</priority>
		</url>';
	}        
        $output .= '</urlset>';
        $mem_end = $this->memory_usage();
	settings_fields( 'sitemap-settings-group' );
